...
function setTimerColor(ratio) {
  ...
  if (ratio >= 0.5) {
    ...
    // 50% → 25%: blanco → amarillo
    const t = 1 - ((ratio - 0.25) / 0.25);
    const [r, g, b] = lerpColor(white, yellow, t);
    bg        = `rgb(${r},${g},${b})`;
    textColor = "#000";
  } else if (ratio > 0) {
    // 25% → 0%: amarillo → rojo
    const t = 1 - (ratio / 0.25);
    const [r, g, b] = lerpColor(yellow, red, t);
    bg        = `rgb(${r},${g},${b})`;
    textColor = t > 0.5 ? "#fff" : "#000";
  } else {
    // 0% en adelante: rojo fijo, parpadeo desde -00:15
    bg        = "rgb(220,30,30)";
    textColor = "#fff";
    if (remainingSeconds <= -15) {
      // Dos parpadeos cada 5 segundos
      // Ciclo de 5s = 5000ms. Dentro del ciclo, parpadea en 0-200ms y 400-600ms
      const cycle = Date.now() % 5000;
      if ((cycle < 200) || (cycle >= 400 && cycle < 600)) {
        bg        = "#fff";
        textColor = "rgb(220,30,30)";
      }
    }
  }
