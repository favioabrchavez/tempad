...
function setTimerColor(ratio) {
  ...
  if (ratio > 0) {
    // De blanco a amarillo en el último 25%
    const t = ratio >= 0.25 ? 0 : 1 - (ratio / 0.25);
    ...
    // Excedido: amarillo → rojo según cuánto se pasó (máx 30s)
    const over = Math.min(Math.abs(remainingSeconds) / 30, 1);
    const yellow = [255, 196, 0];
    const red    = [220, 30, 30];
    const [r, g, b] = lerpColor(yellow, red, over);
    bg        = `rgb(${r},${g},${b})`;
    textColor = over > 0.5 ? "#fff" : "#000";
  }
